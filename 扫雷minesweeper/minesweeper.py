...
class Minesweeper:
    ...
    def create_widgets(self):
        """创建游戏界面组件"""
        # 状态显示标签
        self.status_label = tk.Label(self.master, text=f"mineleft: {self.mines}")
    ...
    def click(self, row, col):
        ...
        if self.mine_grid[row][col] == -1:  # 踩到地雷
            self.game_over = True
            self.show_all_mines()
            messagebox.showinfo("Oh no", "Game over！")
        else:
            self.reveal(row, col)
            if self.check_win():
    ...
    def flag(self, row, col):
        ...
        if not self.revealed[row][col] and not self.game_over:
            ...
            if btn["text"] == "🚩":
                ...
                btn.config(text="🚩")
                self.flags += 1
            self.status_label.config(text=f"mineleft: {self.mines - self.flags}")
